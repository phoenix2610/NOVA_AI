...
VOLUME_GAIN = 1.5    # +50% output volume
...
def synthesise(text: str) -> str:
    ...
    if _kokoro_pipe is not None:
        ...
        if samples:
            ...
            # ── +50% volume boost, hard-clip to avoid distortion ──────────────
            audio_out = np.clip(audio_out * VOLUME_GAIN, -1.0, 1.0)
    ...
    # Boost Piper WAV in-place via sox if available, otherwise paplay volume flag
    try:
        import shutil
        if shutil.which("sox"):
            boosted = tempfile.mktemp(suffix="_nova_tts_boosted.wav")
            subprocess.run(
                ["sox", tmp, boosted, "vol", str(VOLUME_GAIN)],
